...
if args.server:
    ...
    with xmlrpc.client.ServerProxy(f'http://{Address}:{Port}', verbose=True) as proxy:
        # Usando MultiCall para fazer chamadas múltiplas em uma única solicitação
        multicall = xmlrpc.client.MultiCall(proxy)
        multicall.fn_add_integers(5, 4)
        multicall.fn_add_floats(5.5, 4.2)
        multicall.fn_add_arrays([1, 2, 3], [4, 5, 6])
        
        # Executar as chamadas múltiplas
        results = multicall()
